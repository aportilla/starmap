...
Star discs are drawn by `makeStarMeshMaterial` — a Mesh-path shader that takes `uCenter` (in buffer-pixel coords) and `uRadius` uniforms and discards fragments outside the disc via `length(gl_FragCoord.xy − uCenter) > uRadius`. No parity-aware snap is needed because the discs are large and their centers are CPU-positioned at integer pixels by the layout pass. Planets and moons render through `makePlanetMaterial` — a `gl.POINTS`-based shader that keeps the parity-aware center snap and `gl_FragCoord − vCenter` disc discard from the perspective stars path, but replaces the flat fill with one of two procedural textures driven by per-vertex attributes (`aPalette0..2`, `aWeights`, `aMode`, `aSeed`, `aTilt`, `aWaterFrac`, `aIceFrac`): surface mode (`aMode=0`) runs a sphere-projected worley/voronoi pass — every fragment derives latitude + longitude from its disc-local position and hashes into cells whose `(lon, lat)` coords are scaled so disc-center cells stay at `SURFACE_PATCH_PX` while cells toward the limb compress along the foreshortening curve, so the texture wraps a visible globe (Mercury reads as iron-grey + rare-earth rose + silicate rust, mixed by gameplay-grid magnitudes); banded mode (`aMode=1`) treats the disc as the projection of a sphere whose pole is tipped forward by `arcsin(RING_MINOR_OVER_MAJOR)` and whose rotation axis is rotated in-plane by `aTilt` (= `bodyVisualTiltRad(body)`, same helper that drives ring tilt) — bands then trace out arched latitude lines parallel to the ring plane rather than straight horizontal strips. Per-disc band count = `clamp(vRadius × BAND_DENSITY, 6, MAX_BAND_COUNT)` so a Uranus-class disc gets ~30 bands and a Venus-class moon gets ~14, with seed-jittered widths and a chunky horizontal boundary warp so band edges undulate like turbulent zonal flow. Each band picks a palette entry by per-band hash (Jupiter, Titan, Uranus). The per-body palette and mode come from `disc-palette.ts` (see "Procedural disc texture" below). See the "Pixel-perfect rendering" notes below for the shared snap math.
...
- **Surface mode** — when `isBandedAtmosphere(body)` returns false. Palette is resource-driven: top-3 resources by `dominantResources(body, 3)`. World-class color is intentionally absent so the disc reads as gameplay-resource composition rather than rocky/desert/ocean taxonomy — Mercury's iron-grey + rare-earth rose + silicate rust ratio mirrors its mining yields directly. World-class color re-enters only as a flat-fill fallback when a body carries no resource signal at all (procgen edge). The chromophore is rendered separately via the atmospheric passes below, not as a palette slot. Surface texture is a sphere-projected worley/voronoi cell pass (`SURFACE_PATCH_PX` sets the equivalent disc-center pitch) — cells live in `(lon, lat)` on the band-aligned sphere with per-body jittered centers, so the texture wraps a globe and cells foreshorten naturally toward the limb. The pass tracks both the closest (F1) and second-closest (F2) cells so the linea pass below can use F2−F1 as the cell-boundary distance. Each pixel takes its nearest cell's palette pick, so ground cover reads as organic lumpy patches rather than per-pixel salt-and-pepper noise.

  Three layers compose per fragment, with stack order keyed off `surfaceAge`:

    1. `resourceSurface` — the body's bulk character at this fragment. Coarse `CONTINENT_GROUP × CONTINENT_GROUP` continent-cell hashes against `waterFraction` flag "water territory"; warm bodies (`vGlobalness < 0.5`, T above ~225 K) paint those cells as flat `OCEAN_COLOR`, cold bodies don't — liquid surface water requires warmth, and on a frozen body the water IS the ice. Non-water cells run the 1.5b region pass: `REGION_PATCH_FACTOR` fine cells aggregate into a super-cell whose hash discretizes into one of `REGION_BUCKET_COUNT` non-empty subsets of `{palette0, palette1, palette2}`. That subset masks the body's natural weights so each region paints from a different combination of its top-3 resources (Mercury's iron-grey, rare-earth rose, silicate rust separate into spatial regions rather than blending uniformly). Biome stipple (`biomePaintFor(body)`) flips individual land pixels to a pigment color derived from `biosphereArchetype × hostStar.cls`, with coverage density keyed to `biosphereTier`.
    2. `iceLayer` — flat `ICE_COLOR`. Whether ice covers this fragment is decided by `icyHere`: a per-fragment binary mixing two priorities by `vGlobalness` (smoothstep on `avgSurfaceTempK` between `ICE_TEMP_GLOBAL_K = 180 K` and `ICE_TEMP_CAP_K = 270 K`). Cap mode (`vGlobalness ≈ 0`) peaks at the poles via `|latSin|`; global mode (`vGlobalness ≈ 1`) uses a per-cell hash so a cold body gets ice randomly scattered across all latitudes. The threshold is `icePriority > 1 − effectiveIceFrac` where `effectiveIceFrac = mix(vIceFrac, 1.0, smoothstep(0.8, 1.0, vGlobalness))` — on bodies cold enough that liquid surface water is thermodynamically impossible (T ≤ ~206 K, globalness ≥ 0.8) the ice shell covers the entire surface and the iceFraction "deficit" manifests through linea cracks below rather than through visible non-ice cells.
    3. `resourceSubsurface` — the complement-masked 1.5b pick. The complement of the surface region's bucket, run through `pickFromPalette` with the body's own weights. Always computed; consumed by the crater and linea passes as "what's underneath."

  Default composition: `col = icyHere ? mix(resourceSurface, ICE_COLOR, vSurfaceAge) : resourceSurface`. Young icy bodies (Europa, Enceladus) render with ice on top — fresh resurfacing keeps it bright; old icy bodies (Callisto, Ganymede) render with resourceSurface on top — billions of years of impact regolith and radiation darkening bury the ice. Continuous lerp so a mid-age body reads as a hybrid.

  Two feature passes layer on top, both painting `resourceSubsurface`:

    - **Craters (1.5c)** — discrete point features whose existence per crater-seed cell scales with `(1 − surfaceAge)²`. Crater color composes by stack order: young (`craterYoung = resourceSubsurface`) reveals the body's other resources beneath the ice; old (`craterOld = icyHere ? ICE_COLOR : resourceSubsurface`) reveals ice on icy fragments, subsurface where no ice exists. Per-crater region (not per-fragment) drives the subsurface mask, so a crater straddling a region boundary paints one uniform color rather than fracturing along the seam.
    - **Linea (1.5d)** — Voronoi cell-boundary cracks gated on `icyHere && (vIceFrac × vSurfaceAge) > LINEA_BODY_THRESHOLD = 0.5`. F2−F1 worley distance is the cell-boundary metric; fragments within `LINEA_WIDTH_FRAC` of any boundary are eligible, and `LINEA_DENSITY` of edges promote to actual linea via a per-edge hash (commutative key `winnerCell + secondCell`). Europa (0.85 × 0.85 = 0.72) and Enceladus (0.95 × 0.95 = 0.90) fire; Ganymede (0.18), Callisto (0.035), Earth (0.07) suppressed.

  Body brightness is not modulated by a stored albedo scalar — overall reflectance emerges from resource composition + water + ice + biome + cratering + linea + atmospheric layers.
...
- `rocky` cold (`insolationBelow: 0.1`) → CH4 (Titan tholin — Titan's bulk is rocky+icy mantle; `chromophoreFor`'s `ATMOSPHERE_MIN_PRESSURE_BAR` gate keeps airless cold rockies like Callisto chromophore-null)
- `rocky` + biotic carbon-aqueous at microbial+ tier → H2O (Earth cloud cover)
- `ocean` cold (`insolationBelow: 0.1`) → CH4 (Titan-class atmospheres on water-mantle bodies)
- `ocean` → H2O (water clouds, default)
...
The result: ~10 visually distinct atmospheric archetypes spanning the catalog (biotic Earth, ocean world, dust-hazed Mars, Titan, Venus/Io, hot sub-Neptune, Hycean, warm Saturn, cool Jupiter, hot Jupiter, Uranus/Neptune). Curated Sol bodies (Jupiter NH3 / Saturn NH3 / Earth H2O / Mars DUST / Venus SO2 / Titan CH4 / Io SO2) set chromophores explicitly in `bodies.csv`; the rest come from procgen. Mars/Venus/Io are curated rather than procgen because the rule table doesn't fire for them (Venus is rocky+abiotic, Io is lava but airless, Mars is desert+thin-atm).

**Atmospheric rendering passes** (1.3a/b/c in `PLANET-RENDER-PLAN.md`):

- **Outward halo + inward fade** — every body carrying any atmospheric signal paints a stroke-stacked halo extending 0–3 px into space plus an inward fade across the visible limb. The outward width is `log10(surfacePressureBar + 1)` bucketed (gas giants without surface pressure get a fixed `GIANT_RIM_WIDTH_PX`); the inward width is `floor(vRadius × INWARD_RIM_FRACTION)`, radius-driven (not pressure-driven) since edge-on column thickness scales with how much sphere the line-of-sight crosses. **Band widths inside the inward fade grow 1, 2, 3, … px from the limb inward** following the sphere-projection foreshortening curve (`bandIdx = floor((sqrt(1 + 8d) − 1) / 2)`), so the rings get visually wider as you move toward disc center. Each band's opacity comes from the **stroke-stacking** closed form `1 − (1 − α)^stackCount` — mathematically equivalent to painting `stackCount` concentric back-to-front strokes at base alpha α. The innermost (limb-side) band sits under all strokes; the outermost under just the widest. A per-pixel hash dither on `distFromLimb` scatters band boundaries across ±0.75 px so the fade reads as organic haze rather than concentric stripes. The outward halo outputs `vec4(vHazeColor, rimA)` against the planet material's `transparent: true`, so it alpha-blends correctly with rings, moons, and other bodies behind it (the disc-interior fragments still output alpha=1, opaque-equivalent under the standard blend).

- **`bandedRimColor` three-regime decision** picks the rim hue: (1) `HIGH_ALTITUDE_CHROMOPHORES = {CH4, SO2, SILICATE}` are visible at the limb → use `CHROMOPHORE_COLOR` (Titan tholin orange, Venus sulfuric yellow, hot sub-Neptune silicate grey-blue); (2) ice giants → `topGases(body)[0]` so CH4 absorption dominates (Uranus/Neptune cyan-blue); (3) everything else → the lightest atm gas's `GAS_COLOR` via `pickLightestAtmGas(body)` and `GAS_MOLECULAR_WEIGHT` (Jupiter/Saturn → H2 cream — NH3 is deep cloud chemistry, not at the limb). Surface-mode bodies use the simpler haze-chromophore-or-Rayleigh decision.

- **1.3a uniform haze tint** — surface-mode bodies with a haze-class chromophore (CH4, SO2, DUST, SILICATE) get a per-fragment lerp `mix(col, vHazeColor, vHazeTint)` across the whole disc, modeling well-mixed aerosol scattering. Quantized into 3 discrete tint levels.

- **1.3b Rayleigh limb** — surface-mode bodies above `RAYLEIGH_PRESSURE_THRESHOLD` with no haze chromophore get a 1-px sky-cyan rim. Earth's case: H2O routes to cloud patches (below) rather than haze, so Rayleigh fires and Earth reads as "blue-air planet with scattered clouds."

- **1.3c H2O cloud patches** — discrete cloud cells painted over non-icy surface fragments (the `!icyHere` gate suppresses them where ice covers the fragment — white-on-white reads as noise) when `chromophoreGas === H2O`. Implemented as anisotropic worley cells in the equator-aligned frame (`CLOUD_LON_PX / CLOUD_LAT_PX` ≈ 2.4:1 east-west stretch) so cloud silhouettes elongate along the body's tilt axis — wind-swept zonal-flow streaks rather than axis-aligned grid squares.
