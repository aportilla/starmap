...
- **Surface mode** — when `isBandedAtmosphere(body)` returns false. Palette is resource-driven: top-3 resources by `dominantResources(body, 3)`. World-class color is intentionally absent so the disc reads as gameplay-resource composition rather than rocky/ice/desert taxonomy — Mercury's iron-grey + rare-earth rose + silicate rust ratio mirrors its mining yields directly. World-class color re-enters only as a flat-fill fallback when a body carries no resource signal at all (procgen edge). The chromophore is rendered separately via the atmospheric passes below, not as a palette slot. Surface texture is a worley/voronoi cell pass (`SURFACE_PATCH_PX` controls cell pitch) — the disc is divided into per-body jittered cells and each pixel takes its nearest cell's palette pick, so ground cover reads as organic lumpy patches rather than per-pixel salt-and-pepper noise. On top of the cell layer the shader paints four terrain features driven by the body's primary surface attributes: a sphere-projected polar cap covering latitudes past `1 − iceFraction` (Europa at 1.0 fills the whole disc; Earth at 0.1 leaves a small cap; Venus at 0 has none); a coarse-cell ocean pass where every `CONTINENT_GROUP × CONTINENT_GROUP` block of worley cells flips to a flat `OCEAN_COLOR` when its hash falls below `waterFraction` (Earth at 0.71 lands ~71% of continent cells under sea level — cell boundaries inherit the worley jitter so coastlines aren't grid-aligned); a per-pixel biome stipple (`biomePaintFor(body)`) that flips individual land pixels to a pigment color derived from `biosphereArchetype × hostStar.cls`, with coverage density keyed to `biosphereTier`; and a per-cell lightness perturbation amplitude-tapered by `(1 − surfaceAge)` that paints the same cratering / mottling pattern over cap, ocean, and land cells (Mercury and Callisto read as pitted; Io and Enceladus stay smooth — same visual language across rock and ice). Body brightness is not modulated by a stored albedo scalar — overall reflectance emerges from resource composition + water + ice + biome + cratering + atmospheric layers.
