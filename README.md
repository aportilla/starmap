...
- **Surface mode** — when `isBandedAtmosphere(body)` returns false. Palette is resource-driven: top-3 resources by `dominantResources(body, 3)`. World-class color is intentionally absent so the disc reads as gameplay-resource composition rather than rocky/ice/desert taxonomy — Mercury's iron-grey + rare-earth rose + silicate rust ratio mirrors its mining yields directly. World-class color re-enters only as a flat-fill fallback when a body carries no resource signal at all (procgen edge). The chromophore is rendered separately via the atmospheric passes below, not as a palette slot. Surface texture is a sphere-projected worley/voronoi cell pass (`SURFACE_PATCH_PX` sets the equivalent disc-center pitch) — cells live in `(lon, lat)` on the band-aligned sphere with per-body jittered centers, so the texture wraps a globe and cells foreshorten naturally toward the limb. Each pixel takes its nearest cell's palette pick, so ground cover reads as organic lumpy patches rather than per-pixel salt-and-pepper noise. On top of the cell layer the shader paints four terrain features driven by the body's primary surface attributes: a sphere-projected polar cap covering latitudes past `1 − iceFraction` (Europa at 1.0 fills the whole disc; Earth at 0.1 leaves a small cap; Venus at 0 has none); a coarse-cell ocean pass where every `CONTINENT_GROUP × CONTINENT_GROUP` block of worley cells flips to a flat `OCEAN_COLOR` when its hash falls below `waterFraction` (Earth at 0.71 lands ~71% of continent cells under sea level — cell boundaries inherit the worley jitter so coastlines aren't grid-aligned); a per-pixel biome stipple (`biomePaintFor(body)`) that flips individual land pixels to a pigment color derived from `biosphereArchetype × hostStar.cls`, with coverage density keyed to `biosphereTier`; a per-region resource-subset mask that aggregates `REGION_PATCH_FACTOR²` fine cells into super-cells and discretizes each super-cell's hash into one of `REGION_BUCKET_COUNT` non-empty subsets of the body's three palette slots — the mask AND's the body's natural weights so each region paints from a different combination of its top-3 resources, giving genuine composition variance (Mercury's iron-grey, rare-earth rose, silicate rust separate into spatial regions rather than blending uniformly); and discrete crater features whose existence per crater-seed cell scales with `(1 − surfaceAge)²` and whose color comes from the **subsurface mask** — the complement of the crater's region bucket — so impacts read as exposed underlying resource composition (a metals-surface region with rare-earth subsurface shows pink-grey craters on dark grey, a silicate-surface region with metals subsurface shows iron-grey craters on rust). Both the region pass and the crater pass are gated to land — oceans and ice caps stay flat because their color isn't from the resource palette. Body brightness is not modulated by a stored albedo scalar — overall reflectance emerges from resource composition + water + ice + biome + cratering + atmospheric layers.
